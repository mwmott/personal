...
(function(factory) {
    if (typeof define === 'function' && define.amd) {
        define('lib/pym.js/src/pym.js',factory);
    } else {
...
define("lib/icanhaz/ICanHaz.min.js", function(){});
...
define("lib/jquery/dist/jquery.min.js", function(){});
...
define("lib/bootstrap/dist/js/bootstrap.min.js", function(){});

require([
    "lib/pym.js/src/pym.js",
    ...
    // Different highlights for different hashes

    if (window.location.hash) {
        $(document.body).addClass("page-" + window.location.hash.replace("#", ""));
    }
    ...
    // Modal code and removing modal fade for IE

    // This was where I was trying to set the browser to behave differently with IE. The first function (:33) was just
    // removing the 'fade' class, which according to StackOverflow would fix the modal (minus the fade). The modal
    // gets stuck here however, so Scott and I thought there were some added dependencies on the class 'modal' or 'in'.

    // The last block is just my attempts to make a modal from scratch, so that IE uses that instead.
    ...
    // ==== or ============================================
        ...
        // $('#bcObject').css('display', 'hidden');

        // $('a.play').on('click', function(e) {
        //     var id = $(this).attr("data-video");

        //     var obj = ich.bcObject({
        //         id: id
        //     });
        //     $(".video-modal").html(obj);
        //     brightcove.createExperiences();

        //     $(document).click(function(e) {
        //         var target = e.target;

        //         if (!$(target).is('#basicModal') && !$(target).parents().is('#basicModal')) {
        //             $('#basicModal').hide();
        //         }
        //     });

        //      var vidPlayer = $('#bcObject');

        //      if(!vidPlayer.is(':visible')) {
        //          vidPlayer.show();
        //          BCL.addPlayer('video_player', '{{id}}');
        //          brightcove.play();
        //      }
         // });
    //    }
    ...
    $("[data-arm]").hover(
        function() {
            $("#map-container").addClass(this.getAttribute("data-arm"));
        }, function() {
            $("#map-container").removeClass(this.getAttribute("data-arm"));
        }
    );
    ...
    $(document).ready(function() {
        wait(700).done(function() {
            $(".eng-play").attr('id', 'eng-drop');
        }).then(wait(450).done(function() {
            $(".fra-play").attr('id', 'fra-drop');
        })).then(wait(1000).done(function() {
            $(".ger-play").attr('id', 'ger-drop');
        }));
    });

    // play icon turns red based on page-country class
    var red = function(where) {
        wait(2400).done(function() {
            $("." + where + "-play").hide();
            $("." + where + "-red").animate({
                "opacity": 1
            }, 500, "linear");
    ...
    // arm movement based on page-country class

    // var move = function(where) {
    //     var arm = $("." + where + "-arm");
    //     var toggle = function() {
    //         arm.toggleClass("pointer-page-" + where);
    //         wait(1100).done(toggle);
    //     };
    //     toggle();
    // };
    ...
    $(document).ready(function() {
        var arm = $(".eng-arm");
        var toggle = function() {
            arm.toggleClass("pointer-eng");
            wait(1200).done(toggle);
        };
        toggle();
    });
    ...
    $('.england').hover(function() {
        var arm = $('.eng-arm');
        arm.addClass('pointer-rotate-eng');
    }, function() {
        $(".eng-arm").removeClass("pointer-rotate-eng");
    });


    $('.france').hover(function() {
        var arm = $('.fra-arm');
        arm.addClass('pointer-rotate-fra');
    }, function() {
        $(".fra-arm").removeClass("pointer-rotate-fra");
    });


    $('.germany').hover(function() {
        var arm = $('.ger-arm');
        arm.addClass('pointer-rotate-ger');
    }, function() {
        $(".ger-arm").removeClass("pointer-rotate-ger");
    });
